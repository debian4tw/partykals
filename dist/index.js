...
var getRandomWithSpread = function getRandomWithSpread(baseVal, extraRandom) {
  if (!extraRandom) {
    return baseVal;
  }
...
var getRandomColorBetween = function getRandomColorBetween(colMin, colMax) {
  if (!colMax) {
    return colMin ? colMin.clone() : new Color();
  }

  return new Color(getRandomBetween(colMin.r, colMax.r), getRandomBetween(colMin.g, colMax.g), getRandomBetween(colMin.b, colMax.b));
};
/**
...
var getRandomVectorBetween = function getRandomVectorBetween(vecMin, vecMax) {
  if (!vecMax) {
    return vecMin ? vecMin.clone() : new Vector3();
  }

  return new Vector3(getRandomBetween(vecMin.x, vecMax.x), getRandomBetween(vecMin.y, vecMax.y), getRandomBetween(vecMin.z, vecMax.z));
};
/**
 * Lerp between two colors, returning a new color without changing any of them.
 */

var lerpColors = function lerpColors(colA, colB, alpha) {
...
var lerp = function lerp(x, y, alpha) {
...
function () {
  ...
  _createClass(Particle, [{
    ...
    value: function reset() {
      ...
      this.position = getConstOrRandomVector(options.offset || new Vector3()); // set particle's ttl

      this.ttl = getRandomWithSpread(options.ttl || 1, options.ttlExtra) || 1; // set per-particle alpha
      ...
      if (options.fade) {
        ...
        if (options.alpha !== undefined) {
          this.alpha = randomizerOrValue(options.alpha);
        } // shifting alpha?
        else {
            this.startAlpha = randomizerOrValue(options.startAlpha);
            this.endAlpha = randomizerOrValue(options.endAlpha);
          }
      ...
      if (options.scaling) {
        ...
        if (options.size !== undefined) {
          this.size = randomizerOrValue(options.size);
        } // shifting size?
        else {
            this.startSize = randomizerOrValue(options.startSize);
            this.endSize = randomizerOrValue(options.endSize);
          }
      ...
      if (options.rotating) {
        this.rotation = randomizerOrValue(options.rotation || 0);
        this.rotationSpeed = randomizerOrValue(options.rotationSpeed || 0);
      ...
      if (firstUpdate) {
        ...
        if (this.rotation !== null) {
          ...
          if (this.startColor && this.age >= this.startColorChangeAt) {
            this.system.setColor(index, lerpColors(this.startColor, this.endColor, this.startColorChangeAt ? (this.age - this.startColorChangeAt) / (1 - this.startColorChangeAt) : this.age));
          } // set animated alpha


          if (this.startAlpha != null && this.age >= this.startAlphaChangeAt) {
            this.system.setAlpha(index, lerp(this.startAlpha, this.endAlpha, this.startAlphaChangeAt ? (this.age - this.startAlphaChangeAt) / (1 - this.startAlphaChangeAt) : this.age));
          } // set animated size


          if (this.startSize != null && this.age >= this.startSizeChangeAt) {
            this.system.setSize(index, lerp(this.startSize, this.endSize, this.startSizeChangeAt ? (this.age - this.startSizeChangeAt) / (1 - this.startSizeChangeAt) : this.age));
          }
        } // add gravity force
...
function () {
  ...
  function ParticlesMaterial(options) {
    ...
    var uniforms = {
      globalColor: {
        value: new Color(options.color || 0xffffff)
      },
      rendererScale: {
        value: 1
      }
...
function () {
  ...
  function ParticlesSystem(options) {
    ...
    this.hasStarted = false; // get particle options
    ...
    this.particleCount = options.system.particlesCount || 10; // get blending mode

    this.blending = options.particles.blending || "opaque"; // get threejs blending mode
  ...
  _createClass(ParticlesSystem, [{
    ...
    value: function create() {
      ...
      var _this$options = this.options,
          ...
          particles = _this$options.particles,
      ...
      var isTransparent = this.blending !== "opaque"; // create the particle geometry

      this.particlesGeometry = new BufferGeometry(); // set perspective mode

      var perspective = system.perspective !== undefined ? Boolean(system.perspective) : true; // create particles material
      ...
      this.speed = system.speed || 1; // set system starting ttl and other params
      ...
      if (alphas) {
        this.particlesGeometry.setAttribute('alpha', new BufferAttribute(alphas, 1));
      }
      ...
      if (sizes) {
        this.particlesGeometry.setAttribute('size', new BufferAttribute(sizes, 1));
      }

      if (rotations) {
        this.particlesGeometry.setAttribute('rotation', new BufferAttribute(rotations, 1));
      }

      this.particlesGeometry.setDrawRange(0, 0); // set scale

      this.material.setBaseScale(system.scale || 400); // create the particles system

      var particleSystem = new Points(this.particlesGeometry, this.material.material);
      particleSystem.sortParticles = isTransparent; // store particles system
...
function (_Randomizer) {
  ...
  function BoxRandomizer(min, max) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(BoxRandomizer).call(this));
    _this.min = min || new Vector3(-1, -1, -1);
    _this.max = max || new Vector3(1, 1, 1);
    return _this;
  }
  /**
...
function (_Randomizer) {
  ...
  function SphereRandomizer(maxRadius, minRadius, scaler, minVector, maxVector) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(SphereRandomizer).call(this));
  ...
  _createClass(SphereRandomizer, [{
    ...
    value: function generate() {
      ...
      var ret = new Vector3(randMinusToOne(), randMinusToOne(), randMinusToOne()); // clamp values
      ...
      ret.normalize().multiplyScalar(getRandomBetween(this.minRadius, this.maxRadius)); // apply scaler
...
function (_Randomizer) {
  ...
  function ColorsRandomizer(min, max) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(ColorsRandomizer).call(this));
    _this.min = min || new Color(0, 0, 0);
    _this.max = max || new Color(1, 1, 1);
    return _this;
  }
  /**
...
function (_Randomizer) {
  ...
  function MinMaxRandomizer(min, max) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(MinMaxRandomizer).call(this));
    _this.min = min;
    _this.max = max;
    return _this;
  }
